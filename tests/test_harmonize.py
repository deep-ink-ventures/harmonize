...
def test_deposit_coins():
    ...
    coin_contract = w3.eth.contract(address=coin_address, abi=get_coin_abi())
    ...
    endpoint_contract = w3.eth.contract(address=endpoint_address, abi=get_endpoint_abi())

    print("User A", user_a, len(user_a.bytes))
    ...
    tx_hash = coin_contract.functions.approve(endpoint_address, 100).transact({'from': account_a.address})
    w3.eth.wait_for_transaction_receipt(tx_hash)

    # Deposit 100 coins into the harmonize canister
    tx_hash = endpoint_contract.functions.depositErc20(principal_to_bytes32(user_a), coin_address, 100).transact({'from': account_a.address})
    w3.eth.wait_for_transaction_receipt(tx_hash)
...
def test_transfer_coins():
    ...
    response = harmonize.transfer(str(user_a), str(user_b), chain_id, coin_address, "50")
    assert_ok(response)
